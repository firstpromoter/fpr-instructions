...
***Important note: Because of Braintree Webhooks limitation, FirstPromoter can't automatically handle refunds. All other functions are working correctly. In case of a refund, you just need to "Deny" the commission/reward of the affiliate.***
...
For added security, you can create a new User Role on Braintree that allows only read operations. You can skip to Step 2 if you are not concerned about this or you are in the Braintree Sandbox environment.
...
1. Login to Braintree, navigate to Account > My User (on the top menu) and scroll down until you see:
...
2. Copy-paste the data from that table to corresponding fields on the FirstPromoter Braintree Setup form.

### Step 4: Set up the Webhooks

1. Go back to Braintree, go to Settings > Webhooks, and click 'Create New Webhook'
2. In the Destination URL field, add the 'Webhook Endpoint URL' value from FirstPromoter Braintree Setup
3. On Notifications, check only 'Cancelled', 'Charged Successfully', and 'Expired'
4. Click 'Create Webhook'
5. Click 'Check URL' to make sure you copied the destination URL correctly.
